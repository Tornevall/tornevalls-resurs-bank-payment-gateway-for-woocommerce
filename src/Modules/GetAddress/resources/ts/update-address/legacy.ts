...
export class LegacyAddressUpdater {
    ...
    initialize() {
        ...
        jQuery(document).on('update_resurs_customer_type', (ev, customerType) => {
            jQuery.ajax({
    ...
    private rbHandleFetchAddressResponse = (() => {
        ...
        const getNamedFields = (el: HTMLElement): boolean => el.hasAttribute('name');
        const getBillingFields = (el: HTMLInputElement): boolean => el.name.startsWith('billing');
        const getShippingFields = (el: HTMLInputElement): boolean => el.name.startsWith('shipping');
        ...
        const getUsableFields = (obj: { name: string }): boolean => obj.name !== '';

        const mapResursFields = (els: Element[]): { name: string; el: HTMLInputElement }[] =>
            els.map(mapResursField).filter(getUsableFields);

        const getAddressFields = (form: HTMLFormElement | null) => {
            let result: { billing: { name: string; el: HTMLInputElement }[]; shipping: { name: string; el: HTMLInputElement }[] } | null = null;
        ...
        const updateAddressFields = (data: any, customerType: string) => {
            ...
            fields?.billing.forEach((obj) => {
                ...
                if (
                    typeof obj.el.parentNode?.parentNode?.classList === 'object' &&
